...
def test_example_bank_retrieves_relevant_cadquery_snippet(tmp_path, monkeypatch):
    ...
    (example_dir / "phone_stand.py").write_text(
        """
...
""",
    ...
    hits = example_bank.retrieve_example_snippets("phone holder stand", max_snippets=3)

    assert hits
    assert hits[0].path == "repo/examples/phone_stand.py"
...
def test_example_bank_cadquery_only_filters_build123d(tmp_path, monkeypatch):
    ...
    example_dir = root / "repo" / "examples"
    example_dir.mkdir(parents=True)
    (example_dir / "holder_build123d.py").write_text(
        """
from build123d import *

with BuildPart() as phone_holder:
    Box(90, 80, 5)
""",
        encoding="utf-8",
    )
    (example_dir / "holder_cadquery.py").write_text(
        """
import cadquery as cq
...
""",
    ...
    hits = example_bank.retrieve_example_snippets("phone holder", cadquery_only=True)

    assert [hit.path for hit in hits] == ["repo/examples/holder_cadquery.py"]
